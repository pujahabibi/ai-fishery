...
def simple_ctdet_decode(heat, wh, reg=None, cat_spec_wh=False, K=100):
    batch, cat, height, width = heat.size()

    detection = []

    heat = _nms(heat)
    scores, _, clses, _, _ = _topk(heat, K=K)
    for i in range(len(scores[0])):
        if scores[0][i] >= 0.48:
            det_tmp = (scores[0][i], clses[0][i])
            detection.append(det_tmp)
...
def ctdet_decode(heat, wh, reg=None, cat_spec_wh=False, K=100):
    ...
    scores = scores.view(batch, K, 1)
    #print("CLSES", clses[0])
    #print("SCORES", scores[0].size(), scores[0])
    ...
    x = torch.cat([scores, clses], dim=2)
    # print("X[0][0]", x[0][0][0])
    # if x[0][0][0] >= 0.48:
    #     print("LOLOS")
    # else:
